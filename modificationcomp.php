...
if (isset($_POST['modifcomp'])) {
    $colonne = mysqli_real_escape_string($db, $_POST['composant']);
    $idcomp = mysqli_real_escape_string($db, $_POST['idcomp']);
    $nouvelleValeur = mysqli_real_escape_string($db, $_POST['nvcomp']);

    if (empty($colonne) || empty($idcomp) || empty($nouvelleValeur)) {
        array_push($errors, "Tous les champs sont obligatoires");
    }

    if (count($errors) == 0) {
       
        $query_update = "UPDATE composant SET $colonne = '$nouvelleValeur' WHERE idcomposant = '$idcomp'";

       
        if (mysqli_query($db, $query_update)) {
            $_SESSION['success'] = "Composant modifié avec succès";
            header('Location: admin.php'); 
            exit();
        } else {
            array_push($errors, "Erreur lors de la modification du composant : " . mysqli_error($db));
        }
    }
...
<div class="wrapper"><br><br>
    ...
    <div class="container" >
      ...
      <form action="modificationcomp.php" method="post" >
        
            <div class="form-group">
                <label for="modcomposant">MODIFIER UN PRODUIT</label> <br>
        <select class="form-select"  name="composant" id="composant"> 
            <option value="nomcomposant">Nom</option> 
            <option value="descriptioncomp">Description</option> 
            <option value="seuilalertcomp">Seuil D'Alert</option> 
            <option value="prixunitairecomp">Prix Unitaire</option>
            <option value="quantitestockcomp">Quantite Stock</option>
        </select>
            </div>

            <div class="form-group">
                <input type="text" placeholder="ID produit" name="idcomp" class="form-control">
            </div>
            <div class="form-group">
                <input type="text" placeholder="Nouvelle Valeur" name="nvcomp" class="form-control">
            </div>
    
            <div class="form-btn" style="text-align:center">
                <input type="submit" value="Modifier" name="modifcomp" class="btn btn-primary">
            </div>
